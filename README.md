...
```shell
python scripts/summarize_functions.py output/udhcpd_dir895.bin   # Generate summary.json for each function
python scripts/firmware_report.py     output/udhcpd_dir895.bin   # Generate a firmware analysis report under reports/firmware_report.md
```
...
output/<binary>/
  ...
  codeql/
    ...
    db/                                    CodeQL database
  reports/firmware_report.md               Firmware analysis report generated by firmware_report.py, optional
```
...
scripts/                      Standalone post-processing entry points for summarize / firmware_report
...
```shell
python scripts/summarize_functions.py output/udhcpd_dir895.bin   # 获取每函数的 summary.json
python scripts/firmware_report.py     output/udhcpd_dir895.bin   # 总结固件分析报告至 reports/firmware_report.md
```
...
output/<binary>/
  ...
  codeql/
    ...
    db/                                    CodeQL database
  reports/firmware_report.md               固件分析报告，由 firmware_report.py 生成（可选）
```
...
scripts/                      独立后处理入口（summarize / firmware_report）脚本
queries/                      CodeQL 查询示例
