...
def reset_core_outputs(package_dir: Path) -> None:
    for sub in (FUNCTIONS_SUBDIR, REGISTRY_SUBDIR, CODEQL_SUBDIR, CODEQL_DB_SUBDIR):
        ...
        if path.exists():
            shutil.rmtree(path)
    # A previous rejected run must not look current while the next run is in
    # progress.  The new manifest is written only after reconstruction ends.
    (package_dir / "reconstruction" / "completeness_manifest.json").unlink(missing_ok=True)
...
def write_function_outputs(func_dir: Path, artifacts: ReconstructionArtifacts) -> None:
    """Write per-function C source, naming map, and pcode files to ``func_dir``."""
    func_dir.mkdir(parents=True, exist_ok=True)
    if artifacts.raw:
        (func_dir / "raw.c").write_text(_normalize_c(artifacts.raw), encoding="utf-8")
    if artifacts.structured:
    ...
    if artifacts.naming_map:
        (func_dir / "naming_map.txt").write_text(artifacts.naming_map.strip() + "\n", encoding="utf-8")
    if artifacts.pcode:
        (func_dir / "pcode.txt").write_text(artifacts.pcode.strip() + "\n", encoding="utf-8")
    if artifacts.access_evidence:
        (func_dir / "access_evidence.txt").write_text(
            artifacts.access_evidence.strip() + "\n",
            encoding="utf-8",
        )
