...
STANDARD_LIB_STRUCTS: frozenset[str] = frozenset({
    # <sys/stat.h>
    "stat", "stat64", "statvfs", "statvfs64",
    # <netinet/in.h>
    "sockaddr_in", "sockaddr_in6", "in_addr", "in6_addr",
    "ip_mreq", "ipv6_mreq", "in_pktinfo",
    # <sys/socket.h>
    "sockaddr", "sockaddr_storage", "sockaddr_un", "sockaddr_dl",
    "linger", "msghdr", "cmsghdr", "iovec",
    # <sys/time.h>
    "timeval", "timezone", "itimerval",
    # <time.h>
    "tm", "timespec", "itimerspec", "tms",
    # <sys/select.h>
    "fd_set",
    # <signal.h>
    "sigaction", "siginfo", "siginfo_t", "sigevent", "sigevent_t",
    # <pwd.h>
    "passwd",
    # <netdb.h>
    "addrinfo", "hostent", "servent", "protoent", "netent",
    # <sys/wait.h> / <sys/resource.h>
    "rusage", "rlimit", "rlimit64",
    # <sys/file.h> (defensive — not currently #included in stubs.h)
    "flock", "flock64",
    # <shadow.h> (defensive — not currently #included in stubs.h)
    "spwd",
    # <dirent.h>
    "dirent", "dirent64",
    # <sys/utsname.h>
    "utsname",
    # <sys/uio.h> (overlap with <sys/socket.h>)
    # already covered: iovec, msghdr
    # <poll.h>
    "pollfd",
    # <sys/epoll.h>
    "epoll_event",
    # <sys/mman.h>
    # (no struct names commonly reconstructed)
    # <termios.h>
    "termios", "termios2", "winsize",
    # <net/if.h>
    "ifreq", "ifconf", "ifaddrs",
    # <sys/sem.h> / <sys/shm.h> / <sys/msg.h>
    "sembuf", "shmid_ds", "msqid_ds",
    # <sched.h>
    "sched_param",
})
...
def render_types_header(structs: dict[str, dict]) -> str:
    body = [
        ...
        "/* Structs sharing a name with standard-library structs are skipped here;",
        "   their definitions come from the #include'd standard headers in stubs.h. */",
        "",
    ]
    if not structs:
        body.append("/* (no reconstructed structs yet) */")
    else:
        skipped = [name for name in sorted(structs) if name in STANDARD_LIB_STRUCTS]
        for name in sorted(structs):
            if name in STANDARD_LIB_STRUCTS:
                continue
            body.append(f"struct {name};")
        if skipped:
            body.append("")
            body.append(
                "/* Skipped (provided by standard headers in stubs.h via #include): "
                + ", ".join(skipped)
                + " */"
            )
        body.append("")
