...
# Truncation limits for catalog display fields.
_NAMING_MAP_MAX_CHARS = 800
_CALL_SITE_DEFAULT_LIMIT = 24
_SEARCH_CODE_DEFAULT_LIMIT = 30
_SEARCH_SNIPPET_MAX_CHARS = 240
...
class FunctionCatalog:
    ...
    def _naming_map_for(self, package_dir: Path | None, addr: str) -> str:
        ...
        if len(text) > _NAMING_MAP_MAX_CHARS:
            return text[:_NAMING_MAP_MAX_CHARS] + "\n...(truncated)"
        return text
    ...
    def call_sites_for(self, address: str, *, limit: int = _CALL_SITE_DEFAULT_LIMIT) -> list[tuple[str, int, str]]:
    ...
    def search_code(self, pattern: str, *, limit: int = _SEARCH_CODE_DEFAULT_LIMIT) -> list[tuple[str, int, str]]:
        ...
        for addr, node in self.graph.nodes.items():
            for lineno, line in enumerate(node.path.read_text(encoding="utf-8", errors="ignore").splitlines(), 1):
                if lower in line.lower():
                    hits.append((addr, lineno, line.strip()[:_SEARCH_SNIPPET_MAX_CHARS]))
                    if len(hits) >= limit:
                        return hits
        return hits
