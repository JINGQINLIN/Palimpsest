...
- Remove undeclared Ghidra pseudo-symbols (`unaff_*`, `extraout_*`, `in_*`, `unique0x*`, `iVar*`,
  `uVar*`, `piVar*`, `puVar*`, `pcVar*`, `local_*`, `acStack_*`, `aiStack_*`, `auStack_*`):
...
- Bare pointer offset casts (`*(T *)((int)base + 0xNN)`) prevent CodeQL from building
  data-flow edges through that field. Replace with `base->field` whenever the field exists
  in recopilot_types.h; if the struct is missing, report it for the structure pass.
