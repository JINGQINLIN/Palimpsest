...
def _matches_security_keyword(value: Any) -> bool:
...
def _risk_apis(items: list[Any], limit: int = 8) -> list[str]:
    ...
    for item in items:
        ...
        if name in RISK_API_NAMES or _matches_security_keyword(item):
            out.append(str(item))
    return out[:limit]
...
def _threat_snippets(func_dir: Path, max_lines: int = 6) -> list[dict[str, Any]]:
    ...
    for filename in ("named.c", "structured.c"):
        ...
        for lineno, line in enumerate(path.read_text(encoding="utf-8", errors="ignore").splitlines(), 1):
            if _matches_security_keyword(line):
...
def _load_summary(func_dir: Path) -> dict[str, Any] | None:
    ...
    return {
        ...
        "side_effects": [_short(x, 220) for x in _as_list(summary.get("side_effects")) if _matches_security_keyword(x)][:5],
        "risk_strings": [_short(x, 220) for x in _as_list(summary.get("important_strings")) if _matches_security_keyword(x)][:6],
